...
主要功能：
- 4轮主动学习，每轮预算B=150（可配参），每轮重新训练多个不同种子的模型（from scratch fine-tuning）
- 选样阶段采用 ensemble 概率平均（committee）→ 熵/策略打分 → 选Top-B
- 评估阶段输出 mean±std（best-seed 只在内存计算，不再写 best_seed_roundX.json）
- 模式：uncontrolled / controlled-balanced / controlled-optimal
- 记录：少数类指标、π_t、n_e/n_n/n_c、κ_t（uniform/逆频）、几何覆盖ρ_t（max/p95）、自适应生成统计等
- 诊断：difficulty / misclassified 基于验证集分析（不并入训练；本版不落盘 diagnostic_*.json）
- 生成：short_simple / long_simple 可纯生成；或 --gen_mix G 表示每轮先生成G条再从池抽B-G条
- 写盘：保留 round_log.csv 与 run_config.json；删除 selected_*.json、generated_*.jsonl、diagnostic_*.json、best_seed_roundX.json、SUMMARY_INDEX.json

依赖：datasets.py, models.py, train_utils.py, sampling.py, llm_qwen.py
"""
...
def model_tag_from_path(model_dir: str) -> str:
    """根据模型路径自动推断模型标签（BERT/ROBERTA/DISTILBERT/DEBERTA/...），回退为目录名大写。"""
    base = os.path.basename(model_dir).lower()
    if "roberta" in base: return "ROBERTA"
    if "distilbert" in base or "distil-bert" in base: return "DISTILBERT"
    if "deberta" in base: return "DEBERTA"
    if "albert" in base: return "ALBERT"
    if "xlnet" in base: return "XLNET"
    if "electra" in base: return "ELECTRA"
    if "bert" in base: return "BERT"
    return _slug(base).upper() or "MODEL"

def dataset_tag_from_init_dir(init_dir: str) -> str:
    """把 INIT_DIR 的最后一层目录名变成数据集标签（如 ratio_real → RATIO-REAL）。"""
    tail = os.path.basename(os.path.normpath(init_dir))
    return _slug(tail).upper() or "DATA"
...
def build_dataset_and_loader(rows, tokenizer, batch_size: int, max_len: int, shuffle: bool, has_label_maybe=True):
...
def compute_per_class_metrics(golds, preds, n_classes: int = 3):
    acc = accuracy_score(golds, preds)
    prec, rec, f1, _ = precision_recall_fscore_support(golds, preds, labels=list(range(n_classes)), average=None, zero_division=0)
...
def eval_model_on_split(clf: Classifier, rows, batch_size: int, max_len: int):
...
def majority_minority_classes(counts: Dict[str, int]):
    if not counts:
        return [], []
    min_cnt = min(counts.values()); max_cnt = max(counts.values())
...
def predicted_class_distribution(probs: np.ndarray):
...
def apply_balanced_quota_with_predictions(sorted_indices, unlabeled_pool, probs_on_pool: np.ndarray, B: int, start_major: str = "entailment"):
    base = B // 3
    extra = B - base * 3
...
def fit_power_law_errors(n_list: List[int], err_list: List[float]):
    if len(n_list) < 3:
...
def estimate_class_quota_optimal(B: int, n_k: Dict[str, int], err_curve_params: Dict[str, Tuple[float, float, float, float]], eta_k: Dict[str, float]):
...
def compute_coverage_metrics(labeled_rows, unlabeled_rows, ref_model: Classifier, batch_size: int, max_len: int):
    ...
    S = emb_L.astype(np.float32); U = emb_U.astype(np.float32)
    S_sq = (S ** 2).sum(axis=1)
    dmins = []
    chunk = 1024
...
def run(args):
    ...
    # Static tokenizer (from a temp classifier) for loaders
    ...
    # Auto tags for output naming
    model_tag = model_tag_from_path(MODEL_DIR)
    data_tag  = dataset_tag_from_init_dir(INIT_DIR)
    ...
    for strat in strategies:
        for mode in args.modes:
            ...
            # 自动命名输出目录
            exp_name = f"{model_tag}_{data_tag}_{strat}_{mode}"
            ...
            for t in range(1, args.rounds + 1):
                ...
                # (B) Selection
                B = args.budget
                gen_G = max(0, int(G_current)) if is_gen_strategy else 0
                selected_indices: List[int] = []

                # (B1) Generation part (optional; 不落盘生成样本)
                ...
                if rest_B > 0:
                    if strat in ("random", "entropy", "alvin"):
                        ...
                        if mode == "uncontrolled":
                            ...
                            pool_probs = ensemble.predict_proba(ldU)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Unified runner for AL with automatic output naming.")
    # Paths
    ...
    p.add_argument("--run_diagnostics", action="store_true")

    # Adaptive generation (only for gen strategies)
    p.add_argument("--gen_adaptive", action="store_true")
    p.add_argument("--gen_step", type=int, default=15)
    p.add_argument("--gen_min", type=int, default=15)
    p.add_argument("--gen_max", type=int, default=60)

...
class QwenJudge:
    ...
    # ---------------------------
    @torch.no_grad()
    def _generate(self, prompt: str, max_new_tokens: int = 96, temperature: float = 0.0) -> str:
    ...
    def judge_label(self, sample: Dict) -> Optional[str]:
        ...
        if not prem or not hyp:
            return None
        resp = self._generate(PROMPT_JUDGE.format(premise=prem, hypothesis=hyp), max_new_tokens=4, temperature=0.0)
        tok = resp.strip().lower().split()[0] if resp.strip() else ""
        tok = re.sub(r"[^a-z]", "", tok)
        return tok if tok in LABEL_SET else None

    # 你原来的“带重试一致性”的接口（保留向后兼容；不被 sampling 调用）
    ...
    def generate_short_simple_candidate(self) -> Dict:
        ...
        resp = self._generate(PROMPT_GEN_SHORT_SIMPLE, max_new_tokens=128, temperature=0.2)
    ...
    def generate_short_simple(self, n_items: int, max_trials: int = 200) -> List[Dict]:
        ...
        while len(results) < n_items and trials < max_trials:
            trials += 1
            resp = self._generate(PROMPT_GEN_SHORT_WITH_LABEL, max_new_tokens=128, temperature=0.2)
            m = re.search(r"\{.*\}", resp, re.DOTALL)
            if not m:
                continue
            try:
    ...
    def generate_long_simple_candidate(self) -> Dict:
        ...
        resp = self._generate(PROMPT_GEN_LONG_SIMPLE, max_new_tokens=192, temperature=0.4)
        m = re.search(r"\{.*\}", resp, re.DOTALL)
        if not m:
            return {}
        try:

...
def run_active_loop(strategy_name, run_id, seed=42, init_size=200, query_size=100, max_rounds=5):
    ...
    for round in range(max_rounds):
        print(f"\n[DISTILBERT | {strategy_name.upper()} Run {run_id} - Round {round+1}] labeled={len(labeled)} pool={len(pool)}")

        train_path = os.path.join(base_dir, f"temp_distilbert_train_{strategy_name}_r{run_id}.jsonl")
        ...
        output_dir = os.path.join(base_dir, "outputs", f"distilbert_{strategy_name}_run{run_id}_round{round+1}")
        training_args = TrainingArguments(
            ...
            learning_rate=2e-5,
            logging_dir=os.path.join(base_dir, "logs", f"distilbert_{strategy_name}_run{run_id}_round{round+1}"),
            logging_steps=50,
            report_to="none"
        )
        ...
        # 记录日志 CSV
        logfile = os.path.join(base_dir, "logs", f"distilbert_{strategy_name}_run{run_id}.csv")

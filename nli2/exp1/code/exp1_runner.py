...
from sampling import select, entropy_select  # import fallback selector
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 1 runner (1:1:1) for BERT + Active Learning")
    ...
    parser.add_argument("--model_dir", type=str,
                        ...
                        help="Local classifier model directory")
    parser.add_argument("--log_dir", type=str,
                        default="/users/acs24jw/nli_project/mis_exp/logs/BT",
                        help="Directory to save per-round metrics CSV")
    ...
    # Metrics CSV (add 'added_count' for transparency)
    ...
    for r in range(1, args.R + 1):
        ...
        # Strategy selection: may return indices (pool) or new_samples (LLM-based)
        ...
        # 1) If LLM returned generated/judged samples, take them first
        if new_samples is not None and len(new_samples) > 0:
            picked_rows.extend(new_samples)

        # 2) Top up from pool via entropy to reach budget if needed
        ...
        # 3) If selector returned pure pool indices and we didn't pick anything yet
        ...
        # Save remaining pool for traceability
        save_jsonl(os.path.join(strat_out_dir, f"round_{r}_pool_remaining.jsonl"), pool)

        # Retrain on augmented set and eval on dev
        train_ds = NliDataset(train, tokenizer, max_len=args.max_len)
        train_loader = build_loader(train_ds, batch_size=args.batch_size, shuffle=True,
                                    tokenizer=tokenizer, max_len=args.max_len)
        _ = train_with_early_stop(

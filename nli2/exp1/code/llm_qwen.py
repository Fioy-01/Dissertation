...
class QwenJudge:
    ...
    def generate_short_simple(self, n_items: int, max_trials: int = 200) -> List[Dict]:
        ...
        Slight randomness (temperature=0.2) helps diversity while keeping outputs short/simple.
        """
        results = []
        trials = 0
        while len(results) < n_items and trials < max_trials:
            trials += 1
            resp = self._generate(PROMPT_GEN, max_new_tokens=128, temperature=0.2)
